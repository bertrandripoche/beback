...
class LoanerAdapter(options: FirestoreRecyclerOptions<Loaner>, private val context: Context, val mode: String): FirestoreRecyclerAdapter<Loaner, LoanerViewHolder>(options) {
    ...
    override fun onBindViewHolder(holder: LoanerViewHolder, position: Int, loaner: Loaner) {
        holder.updateWithLoaner(loaner, position, context, mode)
    }
